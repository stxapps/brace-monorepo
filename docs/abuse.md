...
- WAF custom rules (Security → WAF → Custom rules).
  ...
  scope to a hostname (http.host eq "extractor.bracemark.com") or path,
...
1. Zone-level Rate Limiting Rules with long windows (Security
   ...
   extractor.bracemark.com in an hour → block that IP for a day." That
...
5. Per-user keying on the remaining bracemark-api routes (minor):
