// Pure tree → rows helpers for the Lists settings section — brace-web's
// `_lists/tree-helpers.ts`, verbatim (see there): the hook gives the same
// ordered forest the sidebar renders; this turns it into the flat, depth-tagged
// row list the table renders and answers the positional questions the
// reorder/move actions ask. No React, no I/O. (Web's dnd-helpers sibling is NOT
// ported — reorder here is buttons-only; a drag surface can bring it later.)

import { LIST_NO_CHILDREN_IDS, type ListItem, type TreeNode } from '@stxapps/shared';
...
// under any id in `collapsedIds`. Order matches the sidebar exactly.
...
// subtree (no cycles), and any no-children container (Trash). The "move to"
// picker subtracts these from the candidate parents.
