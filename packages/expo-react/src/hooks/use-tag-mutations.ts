// Edit operations for tags — the tag sibling of use-list-mutations and the expo
// sibling of web-react's hooks/use-tag-mutations.ts, verbatim in contract (see
// there for how tags differ from lists: NO system tags, and a link's dangling
// `tagIds` reference is allowed — skipped at read time — so destroy needs no
// link guard; `findOrCreate` is the link editor's reuse-or-mint entry point).
...
export interface TagMutations {
  ...
  // A blank name is a no-op (returns null).
  ...
  // Permanently delete a tag. Rejected only for a tag that still has sub-tags —
  // deleting it would silently re-root them (buildTree promotes orphans), so the
  // UI must move them first. The thrown message surfaces to the user.
  destroy: (tag: TagItem) => Promise<void>;
  // Re-rank a sibling group into a new order in one batch (e.g. sort A→Z). Only
  // the tags whose rank actually changes are written — so an already-ordered
  // group is a no-op.
...
export function useTagMutations(): TagMutations {
  ...
  const findOrCreate = useCallback(
    async (name: string) => {
      ...
      // New top-level tag at index 0 — the same landing slot as the Tags
      // settings CreateRow, so the same action lands the same place wherever
      // it's invoked (see web's rationale on recency order).
  ...
  const destroy = useCallback(
    async (tag: TagItem) => {
      ...
      // time, unlike a list that must shed its links first.
      const tags = await readTags();
  ...
  const reorder = useCallback(
    async (ordered: TagItem[]) => {
      ...
      // Sequential, not Promise.all — same reasoning as useListMutations.reorder.
      let wrote = false;
