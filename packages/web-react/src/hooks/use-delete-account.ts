...
// The delete-account submit sequence (Settings → Account → Delete account) —
// the same door-fetch → unwrap → sign shape as use-sign-in, but bound to the
// `delete-account` action and finishing with the LOCAL teardown instead of a
// session mint. The password re-entry is the point, not a formality: the server
// refuses this call without a fresh proof signed by the DEK-derived key, so a
// stolen bearer token alone can never erase an account (docs/data-lifecycle.md).
// Web-only for the same reason as use-sign-in: it reaches for web-only crypto
// and the web auth context.

// The server's 409: a subscription that would keep billing (renewing, or in
// dunning). Typed so the form can point the user at the Paddle portal (the
// Subscription section) rather than showing a generic failure.
